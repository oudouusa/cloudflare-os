...
Not yet accepted: a non-duplicated post-index-fix **real-model** DeepSeek in-app
normal response and real-model-selected Gadget file/tool/test execution.
...
| 4 | Real owner-selected OpenCode Go normal and agent tool flow | Partial | Historical GLM proof, strict bridge suites, and the isolated full Cloudflare OS mock all pass. The first two real DeepSeek in-app streams exposed LiteLLM's empty-choice bug; the ASB tool flow then succeeded but exposed missing reasoning replay. A later real normal response completed HTTP 200/`agent.run=ok` with reasoning persisted, but revealed that the late-reasoning item reused the active message index and duplicated final text. Commit `1ce5b2a` isolates that item at the next free output index and passes the exact production pi-ai parser plus mock regression coverage. A non-duplicated live confirmation and real-model-selected tool/artifact/test flow remain pending. |
